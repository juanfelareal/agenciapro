...
export default function PortalProjects() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-ink-900">Mis Proyectos</h1>
        <p className="text-ink-500 mt-1">Revisa el progreso de tus proyectos</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="relative">
          ...
          <select
            ...
            className="pl-10 pr-8 py-2.5 bg-white border border-ink-200 rounded-xl appearance-none
                     focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
          >
      ...
      {filteredProjects.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProjects.map((project) => (
            <Link
              ...
              className="bg-white rounded-2xl border border-ink-100 shadow-soft p-6
                       hover:shadow-md hover:border-ink-200 transition-all group"
            >
              ...
              <h3 className="font-semibold text-ink-900 mb-2 group-hover:text-violet-600 transition-colors">
                {project.name}
              </h3>
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-ink-500">Progreso</span>
                  <span className="font-medium text-ink-900">{project.progress || 0}%</span>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-ink-100 flex items-center justify-between">
                <span className="text-sm text-ink-500">Ver detalles</span>
                <ArrowRight className="w-4 h-4 text-ink-400 group-hover:text-violet-600 group-hover:translate-x-1 transition-all" />
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-ink-100 p-12 text-center">
          <div className="w-16 h-16 bg-ink-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <FolderKanban className="w-8 h-8 text-ink-400" />
          </div>
          <h3 className="text-lg font-semibold text-ink-900 mb-2">No hay proyectos</h3>
