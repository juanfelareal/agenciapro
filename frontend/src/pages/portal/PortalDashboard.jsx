...
export default function PortalDashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-ink-900 to-ink-800 rounded-3xl p-8 text-white">
        <h1 className="text-2xl sm:text-3xl font-bold">
          ¡Hola, {client?.name?.split(' ')[0]}! 👋
        </h1>
        {welcomeMessage ? (
          <p className="mt-2 text-ink-200">{welcomeMessage}</p>
        ) : (
        ...
        {hasPermission('can_view_projects') && (
          <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-ink-900">{data?.projects?.total || 0}</span>
            </div>
            <h3 className="mt-4 font-medium text-ink-900">Proyectos</h3>
        ...
        {hasPermission('can_view_tasks') && (
          <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-ink-900">{data?.tasks?.total || 0}</span>
            </div>
            <h3 className="mt-4 font-medium text-ink-900">Tareas</h3>
            <p className="text-sm text-ink-500">
        ...
        {hasPermission('can_approve_tasks') && (
          <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-ink-900">{data?.tasks?.pending_approval || 0}</span>
            </div>
            <h3 className="mt-4 font-medium text-ink-900">Por Aprobar</h3>
            <p className="text-sm text-ink-500">
        ...
        {hasPermission('can_view_invoices') && (
          <div className="bg-white rounded-2xl p-6 border border-ink-100 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-ink-900">{formatCurrency(data?.invoices?.paid_amount)}</span>
            </div>
            <h3 className="mt-4 font-medium text-ink-900">Pagado</h3>
            <p className="text-sm text-ink-500">
        ...
        <div className="bg-white rounded-2xl border border-ink-100 shadow-soft overflow-hidden">
          <div className="px-6 py-4 border-b border-ink-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-semibold text-ink-900">Esperando tu Aprobación</h2>
                <p className="text-sm text-ink-500">Tareas que necesitan tu revisión</p>
              </div>
            </div>
            <Link
              to="/portal/tasks?requires_approval=1"
              className="text-sm text-ink-600 hover:text-ink-900 flex items-center gap-1"
            >
            ...
            {data.pending_approval.slice(0, 5).map((task) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-ink-500">{task.project_name}</p>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-ink-100 shadow-soft overflow-hidden">
          <div className="px-6 py-4 border-b border-ink-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-semibold text-ink-900">Actividad Reciente</h2>
                <p className="text-sm text-ink-500">Últimas actualizaciones en tus tareas</p>
              </div>
            </div>
            <Link
              to="/portal/tasks"
