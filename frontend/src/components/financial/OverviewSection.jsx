            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={monthlyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: '#f4f4f5', marginBottom: '8px' }}
        ...
        >
          <div className="h-44 relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col">
              ...
              <div className="text-xs text-[#5c5d66]">Total</div>
            </div>
          </div>
          <div className="flex flex-wrap gap-4 mt-4">
            {expenseBreakdown.map((item) => (
              <div key={item.name} className="flex items-center gap-2 text-xs text-[#94959c]">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                ...
                {plData.months.map((m) => (
                  <th key={m} className="text-right px-4 py-3 text-[11px] font-semibold text-[#5c5d66] uppercase tracking-wider border-b border-white/5">
                    {m}
                  </th>
                ))}
            ...
            <tbody>
              {/* Ingresos */}
              <tr>
                <td className="px-4 py-3 border-b border-white/5 font-semibold">Ingresos</td>
                {plData.ingresos.map((v, i) => (
                  <td key={i} className="px-4 py-3 text-right border-b border-white/5">{formatCurrency(v)}</td>
                ))}
                <td className="px-4 py-3 text-right border-b border-white/5 bg-[#13141a] font-semibold">
                  {formatCurrency(plData.ingresos.reduce((a, b) => a + b, 0))}
                </td>
              </tr>

              {/* Gastos */}
              <tr className="bg-[#13141a]">
                <td className="px-4 py-3 border-b border-white/5 font-semibold">Gastos</td>
                {plData.gastos.map((v, i) => (
                  <td key={i} className="px-4 py-3 text-right border-b border-white/5">{formatCurrency(v)}</td>
                ))}
                <td className="px-4 py-3 text-right border-b border-white/5 bg-[#0c0d10] font-semibold">
                  {formatCurrency(plData.gastos.reduce((a, b) => a + b, 0))}
                </td>
              </tr>

              {/* Nómina Fija */}
              <tr className="text-[#5c5d66] text-xs">
                <td className="px-4 py-2 pl-8 border-b border-white/5">Nómina Fija</td>
                {plData.nomina.map((v, i) => (
                  <td key={i} className="px-4 py-2 text-right border-b border-white/5">{formatCurrency(v)}</td>
                ))}
                <td className="px-4 py-2 text-right border-b border-white/5 bg-[#13141a]">
              ...
              {/* Comisiones */}
              <tr className="text-[#5c5d66] text-xs">
                <td className="px-4 py-2 pl-8 border-b border-white/5">Comisiones</td>
              ...
              {/* Operación */}
              <tr className="text-[#5c5d66] text-xs">
                <td className="px-4 py-2 pl-8 border-b border-white/5">Operación</td>
              ...
              {/* Otros */}
              <tr className="text-[#5c5d66] text-xs">
                <td className="px-4 py-2 pl-8 border-b border-white/5">Otros</td>
              ...
              <tr className="bg-teal-500/10">
                <td className="px-4 py-3 border-b border-white/5 font-semibold text-teal-400">= Utilidad</td>
                {plData.utilidad.map((v, i) => (
                  <td key={i} className="px-4 py-3 text-right border-b border-white/5 text-teal-400">{formatCurrency(v)}</td>
                ))}
                <td className="px-4 py-3 text-right border-b border-white/5 bg-teal-500/20 font-semibold text-teal-400">
              ...
              {/* Margen */}
              <tr>
                <td className="px-4 py-3 border-b border-white/5">Margen %</td>
                {plData.margen.map((v, i) => (
                  <td key={i} className="px-4 py-3 text-right border-b border-white/5 text-teal-400">{formatPercent(v)}</td>
                ))}
                <td className="px-4 py-3 text-right border-b border-white/5 bg-[#13141a] font-semibold text-teal-400">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        >
          <div className="grid grid-cols-3 gap-3 mt-5">
            <div className="bg-[#13141a] rounded-xl p-4 text-center">
              <div className="text-xs text-[#5c5d66] mb-2">2024</div>
              <div className="text-xl font-bold">{formatCurrency(186000000)}</div>
              <div className="text-xs text-[#5c5d66] mt-1">Base</div>
            </div>
            <div className="bg-[#13141a] rounded-xl p-4 text-center">
              <div className="text-xs text-[#5c5d66] mb-2">2025</div>
              <div className="text-xl font-bold">{formatCurrency(312000000)}</div>
              <div className="text-xs text-teal-400 mt-1">+68%</div>
            </div>
            <div className="bg-[#13141a] rounded-xl p-4 text-center">
              <div className="text-xs text-[#5c5d66] mb-2">2026</div>
              <div className="text-xl font-bold">{formatCurrency(462000000)}</div>
              <div className="text-xs text-teal-400 mt-1">+48%</div>
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="h-2 bg-[#13141a] rounded-full overflow-hidden">
              <div className="h-full bg-teal-400 rounded-full transition-all duration-500" style={{ width: '64%' }} />
            </div>
            <div className="text-xs text-[#5c5d66] mt-2">64% ejecutado • Faltan {formatCurrency(258000000)}</div>
          </div>
        </FinancialCard>
      </div>
    </div>
