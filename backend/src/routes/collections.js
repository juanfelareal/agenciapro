...
router.get('/summary', async (req, res) => {
  try {
    ...
    // Get recently paid (last 30 days)
  ...
  try {
    ...
    let invoiceQuery = `
      ...
      WHERE i.client_id = ?
        ...
        AND i.organization_id = ?
    `;
    const invoiceParams = [client_id, req.orgId];
    ...
    if (invoices.length === 0) {
      return res.status(400).json({ error: 'No hay facturas pendientes para este cliente' });
    }

    const totalOwed = invoices.reduce((sum, inv) => sum + inv.amount, 0);

    // Get organization info for branding
    const org = await db.get(`SELECT name, logo_url FROM organizations WHERE id = ?`, [req.orgId]);
    ...
    const defaultMessage = `Esperamos que te encuentres bien. Te enviamos el estado de cuenta actualizado de ${clientDisplayName} con ${orgName}.`;
    const messageBody = custom_message || defaultMessage;

    const emailSubject = subject || `Estado de Cuenta - ${clientDisplayName} | ${orgName}`;
                ...
                <tr>
                  <td style="padding: 0 40px 32px;">
                    <p style="color: #374151; font-size: 14px; line-height: 1.6; margin: 0;">
                      Si ya realizaste el pago, por favor envianos el comprobante para actualizar tu estado de cuenta. Si tienes alguna duda, no dudes en contactarnos.
                    </p>
                  </td>
                </tr>
    ...
    await transporter.sendMail({
      from: `"Estefania Hernandez - ${orgName}" <${process.env.EMAIL_USER}>`,
      to: email_to,
