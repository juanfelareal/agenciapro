...
export async function upsertDailyMetrics(clientId, date, metrics) {
  ...
  if (existing) {
    ...
    `).run(
      ...
      metrics.fb_link_clicks || 0,
      metrics.fb_add_to_cart || 0,
...
export async function syncClientForDate(clientId, date) {
  ...
  const metricsToSave = {
    ...
    fb_link_clicks: fbMetrics.linkClicks,
    fb_add_to_cart: fbMetrics.addToCart,
...
export async function syncAllClientsForDate(date = null) {
  if (!date) {
    date = getColombiaDate(0); // Today — cron runs every 5 min for near real-time data
  }
